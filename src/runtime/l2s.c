 ...
 *
 * The naming algebra follows the on-disk convention established by 参考实现's
 ...
 * upstream is a 1342-line ptrace-based extension using talloc and 参考实现's
 ...
 * treated as unset rather than silently mis-matched, because 参考实现's own
              ...
              char *base, size_t basesz)
{
    ...
    /* 参考实现 does not split at all: it copies the first
    ...
    /* "." and ".." are never linkable: 参考实现's move_and_symlink_path()
 ...
 *     "/d" + base -> "/d/" base
 *
 * This mirrors 参考实现, which builds the intermediate with
 ...
 * what 参考实现's strncpy would have produced and is what the builder below
...
/* Write `value` as exactly `digits` decimal digits, zero padded.  参考实现
 ...
 * handed to a normal build and vice versa, and 参考实现's own
 ...
 *
 * The 4-digit fields are anchored at the END of the name.  参考实现 never
                          ...
                          l2s_info *out)
{
    ...
    if (kind == L2S_KIND_NONE) {
        if (bodylen > L2S_DIGITS &&
            ...
            /*
             * 参考实现's is_l2s_file() only checks the shape
                           ...
                           char *out_orig)
{
     ...
     * 参考实现的元数据条目) is not a faked hard link. */
                  ...
                  char *out_orig, l2s_info *out)
{
     ...
     * entry of the l2s directory, and 参考实现 itself cannot tell the two
     * layouts apart in that case either (its is_l2s_file() looks only at
     * the basename).  Preferring the l2s directory there is what 参考实现's
    ...
    if (l2s_dir == NULL)
        ...
        dir_is_orig = 1;   /* some other directory: not 参考实现's l2s dir */
    else
                      ...
                      l2s_paths *out, char *out_mid, char *out_final)
{
     ...
     *
     * `head` is that directory plus its separator, exactly as 参考实现's
     * strncpy would leave it, and is also what bounds the total length.
     */
    if (l2s_dir != NULL) {
        /* 参考实现's own bound, transcribed from the strlen form so that no
        ...
        /* 参考实现 checks only strlen(PREFIX) + strlen(original) + 5 >= PATH_MAX,
     ...
     * ".NNNN" = 5 more for the final.  参考实现 never checks this -- it
...
int l2s_next_final(const char *name, char *out, size_t outsz)
{
    ...
    /* Everything up to the final '.' is kept verbatim: 参考实现 does
                   ...
                   unsigned int *out_count)
{
    ...
    /* 参考实现's decrement_link_count(): when the count reaches 0 both the
...
/* 参考实现's atoi(final + strlen(final) - 4): the count is the last four
...
static int nlink_from_path(const char *path, unsigned int *out)
{
    ...
    /* 参考实现 accepts either the final file itself or the intermediate that
                        ...
                        unsigned int *out_generation)
{
    ...
    /* 参考实现 leaves intermediate_suffix at 1000 and then strcpy()s the
     * 1000-suffixed name it just formatted, so it proceeds with a
     * generation 参考实现 itself considers occupied.  Report the exhaustion
