 ...
 * cannot do").
 *
 * The rules implemented here are transcribed from the upstream PRoot
 * extension src/extension/link2symlink/link2symlink.c, and cross-checked
 * against a live artifact of the PRoot instance this container runs under
 * (see L2S_GROUND_TRUTH_* below and test_l2s.c).
 *
 * Reference (upstream PRoot, GPL-2.0+), the lines this module mirrors:
 *
...
/* Number of decimal digits PRoot uses for the generation and for the link
...
/* The two spellings of the PRoot prefix.  A normal PRoot build uses
...
/* Directory PRoot keeps its intermediates in when PROOT_L2S_DIR is set.
...
typedef enum {
    /* PRoot, the format this module is written against. */
    L2S_SCHEME_PROOT = 0,

    /* proroot's own metadata tree under /.proroot-meta: one 16-hex-char key
...
typedef struct {
     ...
     * the file it stands for.  Must not have a trailing slash: PRoot strips
    ...
    /* Mirror PRoot's is_l2s_file() exactly, including its misclassification
 ...
 *
 * Equivalent to PRoot's first-link branch with an empty l2s directory:
 ...
 * (PRoot's literal ".0002" is only reachable through a chain of two
 ...
 *   - a proroot meta entry "g_<16hex>" / "m_<16hex>"/"<16hex>[_<digits>]"
 *
 ...
 *
 * PRoot computes the faked count from the *file name*
 *     finalStat.st_nlink = atoi(final + strlen(final) - 4);
 ...
 * intermediates.  PRoot probes with access(F_OK), which FOLLOWS symlinks,
...
/*
 * A real artifact produced by the PRoot instance this container runs
