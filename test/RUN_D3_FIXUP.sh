#!/bin/sh
# D3 修复单元测试：readlink_fixup 判别矩阵（源码级，见 probe_d3_fixup.c 头注）
# 用法：sh test/RUN_D3_FIXUP.sh
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-gcc}"
OUT="/tmp/d3fixup-probe.$$"

echo "▶️  D3 /proc 泄漏反向翻译（源码级单元）"

# gcc13 间歇 ICE：重试 + 降优化级（与 BUILD_RUNTIME.sh 同策略）
OPT="-O2"
for i in 1 2 3 4; do
    if $CC $OPT -Wall -Wextra -Wformat=2 -Wno-nonnull-compare -Wno-unused-parameter \
        ...
        -o "$OUT" "$ROOT/test/probe_d3_fixup.c" "$ROOT/src/l2s/l2s.c" \
