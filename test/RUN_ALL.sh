...
# 8b. l2s **散落布局**的端到端覆盖
#
# 【为什么必须单独加这一条】`tools/bxroot-run` 现在会给
# `BXROOT_L2S_DIR` 兜底（默认 `<rootfs>/.l2s`，与 launcher 对齐），
# 所以**不设该变量已经不再走散落布局**了 —— 散落这条端到端路径
# 因此失去了覆盖（只有 test_l2s_rt.c 用注入的内存 FS 在测，
# 它走的路径与真实 stat 钩子不同，测不出布局问题）。
#
# 本步显式把 `BXROOT_L2S_DIR` 清空，强制走散落布局，再验一次
# 硬链接契约：中间层生成在客户文件旁边，但 stat/lstat 必须仍然
# 报 nlink=2、非符号链接、size 是真实内容长度。
# ---------------------------------------------------------------------
if [ -f test/RUN_L2S_SCATTERED.sh ]; then
    run_step "l2s 散落布局端到端" sh test/RUN_L2S_SCATTERED.sh
else
