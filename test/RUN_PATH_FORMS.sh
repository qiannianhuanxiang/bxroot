...
static void t06(void) {
    ...
    fd = open(abs, O_RDONLY);
    if (fd < 0) { bad("T06", "经绝对符号链接 open", strerror(errno)); goto out; }
    close(fd);
...
# guest 视角：rootfs = /，工作目录 = B_DIR（用宿主真实路径，因为 bxroot
# 要的是"翻译前"的路径）。探针本身也复制到 B_DIR 内，用 guest 视角调用。
cp "$PROBE" "$B_DIR/pathform"
( cd "$B_DIR" && "$LAUNCHER" -r / -w "$B_DIR" "/$B_DIR/pathform" ) \
    > "$WORK/B.out" 2>&1
B_RC=$?
sed 's/^/   /' "$WORK/B.out"
echo "   （退出码 $B_RC）"
...
if [ "${B_FAIL:-1}" -ne 0 ]; then
    echo "❌ bxroot 侧有 $B_FAIL 个用例失败："
